...
const LogsScreen = () => {
  ...
  useEffect(() => {
    if (prioritizePlate && rowRefs.current[prioritizePlate]) {
      rowRefs.current[prioritizePlate].scrollIntoView({ behavior: "smooth", block: "center" });
    }
  ...
  const handleExport = () => {
    try {
      const exportData = sortedData.map((item, index) => ({
        ...
        "Waktu Keluar": item.exit_time ? formatDateTime(item.exit_time) : "Belum Keluar",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center text-sm text-gray-600">
        <span>Menampilkan {sortedData.length} dari {logsData.length} log parkir</span>
        <div className="flex gap-4">
          <span className="flex items-center">
            <div className="w-3 h-3 bg-orange-400 rounded-full mr-2"></div>
            Sedang Parkir: {logsData.filter(log => log.status === "PARKIR").length}
          </span>
          <span className="flex items-center">
              ...
              {sortedData.length === 0 ? (
                ...
                sortedData.map((log) => (
                  <tr
                    key={log.id}
                    ref={el => {
                      if (log.plate_number === prioritizePlate) rowRefs.current[log.plate_number] = el;
                    }}
                    className={`hover:bg-gray-50 transition-colors ${log.plate_number === prioritizePlate ? 'bg-yellow-100' : ''}`}
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {log.photo ? (
                        <button
                          onClick={() => openImageModal(log.photo, log.plate_number)}
